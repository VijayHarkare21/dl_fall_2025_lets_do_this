...
def adanpc_refinement(
    ...
    k: int = 50,
    threshold: float = 0.9,  # Base confidence threshold (Pass 1)
    dataset_name: str = "dataset"
) -> np.ndarray:
    """
    AdaNPC-style refinement with one extra refinement pass:

    Pass 1 (Online adaptation):
      - Memory M initialized with all source (train+val) features + labels.
      - Process each test feature x_u one-by-one:
          1. Compute cosine similarity between x_u and all items in M.
          2. Find k-NN in M.
          3. Aggregate neighbor similarities per class → class_scores.
          4. Softmax(class_scores) → η_k(x_u).
          5. Predict y_hat = argmax_c η_k(x_u)[c].
          6. If max_c η_k(x_u)[c] > threshold, append (x_u, y_hat) to M.

    Pass 1.5 (Offline refinement, higher threshold):
      - Freeze M from Pass 1.
      - For all test features, recompute η_k(x_u) using current M.
      - For any x_u with max_c η_k(x_u)[c] > high_threshold,
        further augment M with (x_u, y_hat).

    Pass 2 (Final prediction with frozen, fully adapted memory):
      - Freeze M after Pass 1.5.
      - For all test features, recompute η_k(x_u) using final M.
      - Return argmax_c η_k(x_u)[c] as final predictions.

    Assumes train_feats and test_feats are already L2-normalized row-wise.
    """
    print(f"\n{'='*40}")
    print(f"Running AdaNPC (Pass1 + Pass1.5 + Pass2) for {dataset_name}")
    print(f"Initial Memory Size: {len(train_labels)}")
    print(f"Base Threshold (Pass 1): {threshold}")
    print(f"{'='*40}")
    ...
    # -------------------------------------------------------------------------
    # Pass 1: Online adaptation with confident pseudo-labels (threshold)
    # -------------------------------------------------------------------------
    print("\n[Pass 1] Online adaptation with memory augmentation...")
    num_confident_added_p1 = 0
    ...
    for idx in range(num_test):
        ...
        if pred_conf > threshold:
            ...
            num_confident_added_p1 += 1

    print(f"[Pass 1] Total test samples: {num_test}")
    print(f"[Pass 1] Confident samples added to memory: {num_confident_added_p1} "
          f"({(num_confident_added_p1 / max(1, num_test)) * 100:.1f}%)")
    print(f"[Pass 1] Memory Size after adaptation: {len(memory_labels)}")

    # -------------------------------------------------------------------------
    # Pass 1.5: Offline refinement with higher threshold
    # -------------------------------------------------------------------------
    # Use a stricter threshold here to avoid propagating noisy labels
    high_threshold = max(threshold, 0.95)
    print(f"\n[Pass 1.5] Offline refinement with high threshold = {high_threshold:.3f}...")

    # Precompute similarity matrix for current memory: (N_t, N_mem)
    sims_all_p15 = test_feats @ memory_feats.T
    N_mem_p15 = memory_feats.shape[0]
    max_label_p15 = int(memory_labels.max())

    num_confident_added_p15 = 0

    for idx in range(num_test):
        sims = sims_all_p15[idx]  # (N_mem_p15,)

        k_eff = min(k, N_mem_p15)
        if k_eff == N_mem_p15:
            neighbor_idx = np.argsort(-sims)
        else:
            neighbor_idx = np.argpartition(-sims, k_eff - 1)[:k_eff]

        neighbor_labels = memory_labels[neighbor_idx]
        neighbor_weights = sims[neighbor_idx]

        class_scores = np.zeros(max_label_p15 + 1, dtype=np.float32)
        np.add.at(class_scores, neighbor_labels, neighbor_weights)
        ...
        if pred_conf > high_threshold:
            memory_feats = np.vstack([memory_feats, test_feats[idx][None, :]])
            memory_labels = np.concatenate(
                [memory_labels, np.asarray([pred_label], dtype=memory_labels.dtype)]
            )
            num_confident_added_p15 += 1

    print(f"[Pass 1.5] Additional confident samples added: {num_confident_added_p15} "
          f"({(num_confident_added_p15 / max(1, num_test)) * 100:.1f}%)")
    print(f"[Pass 1.5] Memory Size after second adaptation: {len(memory_labels)}")

    # -------------------------------------------------------------------------
    # Pass 2: Final prediction with frozen adapted memory
    # -------------------------------------------------------------------------
    print("\n[Pass 2] Final prediction with frozen adapted memory...")

    # Precompute similarity matrix with final memory
    sims_all = test_feats @ memory_feats.T
    N_mem_final = memory_feats.shape[0]
    max_label_final = int(memory_labels.max())  # number of classes inferred from memory

    final_preds: List[int] = []

    for idx in range(num_test):
        sims = sims_all[idx]  # (N_mem_final,)

        k_eff = min(k, N_mem_final)
        if k_eff == N_mem_final:
            neighbor_idx = np.argsort(-sims)
        else:
            neighbor_idx = np.argpartition(-sims, k_eff - 1)[:k_eff]
        ...
        class_scores = np.zeros(max_label_final + 1, dtype=np.float32)
        np.add.at(class_scores, neighbor_labels, neighbor_weights)
    ...
    print(f"[Pass 2] Completed final predictions for {num_test} samples.")
...
def parse_args():
    ...
    parser.add_argument('--backbone', type=str, default='vit', choices=['vit', 'wideresnet', 'convnextv2_tiny'])
    parser.add_argument('--img_size', type=int, default=96)
    parser.add_argument('--k_values', type=int, nargs='+', default=[1, 5, 10, 20, 50, 100, 200])
