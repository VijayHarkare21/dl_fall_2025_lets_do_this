...
class EvalImageDataset(Dataset):
    def __init__(
        ...
        # 1. The "Safe" View (Original Squashed) - Replicates training exact match
        self.transform_base = T.Compose([
            T.Resize((crop_size, crop_size)), # Explicit squash
            T.ToTensor(),
            self.normalize
        ])
        
        # 2. The "TTA" Views (TenCrop)
        # We squash to slightly larger, then crop. 
        # crop_size / 0.875 is standard, e.g. 96 -> 109
        resize_dim = int(crop_size / 0.875) 
        self.transform_crops = T.Compose([
            T.Resize((resize_dim, resize_dim)), # Squash to larger
            T.TenCrop(crop_size),               # 10 Crops
            # Stack and normalize
            T.Lambda(lambda crops: torch.stack([
                self.normalize(T.ToTensor()(crop)) for crop in crops
            ]))
        ])
    ...
    def __getitem__(self, idx):
        ...
        if self.use_tta:
            # --- COMBINED STRATEGY (12 Views) ---
            views = []
            
            # A. Base View (1) + Flip (1) = 2 Views
            # This ensures we have the "whole bird" view which is critical
            img_base = self.transform_base(image)
            img_base_flip = self.transform_base(TF.hflip(image))
            views.extend([img_base, img_base_flip])
            
            # B. TenCrop Views (10)
            # These provide zoomed-in/shifted variance
            crops_tensor = self.transform_crops(image) # (10, C, H, W)
            # Unbind to list
            views.extend([c for c in crops_tensor])
...
def evaluate_dataset(
    ...
    # If 12 views, effective batch size is batch_size * 12.
    eff_batch_size = max(1, batch_size // 12)
