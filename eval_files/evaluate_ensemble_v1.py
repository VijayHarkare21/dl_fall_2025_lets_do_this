...
- WideResNet and ConvNeXt backbones
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # Checkpoint
    parser.add_argument(
        '--checkpoint1', type=str, required=True,
        help='Path to model 1 checkpoint'
    )
    parser.add_argument(
        '--checkpoint2', type=str, required=True,
    ...
    parser.add_argument(
        '--backbone1', type=str, default=None,
        choices=['convnextv2_tiny', 'wideresnet'],
        help='Backbone 1 architecture'
    )
    parser.add_argument(
        '--backbone2', type=str, default=None,
        choices=['convnextv2_tiny', 'wideresnet'],
        help='Backbone 2 architecture'
    )
    parser.add_argument(
...
def evaluate_dataset(
    ...
    # Extract features
    train_features1, train_labels, _ = extract_features_from_dataloader(
        backbone1, device, train_loader, 'train', has_labels=True
    )
    val_features1, val_labels, _ = extract_features_from_dataloader(
        backbone1, device, val_loader, 'val', has_labels=True
    )
    train_features2, train_labels, _ = extract_features_from_dataloader(
        backbone2, device, train_loader, 'train', has_labels=True
    )
    val_features2, val_labels, _ = extract_features_from_dataloader(
        backbone2, device, val_loader, 'val', has_labels=True
    )
    ...
    if has_test and output_csv is not None:
        ...
        test_features1, _, test_filenames = extract_features_from_dataloader(
            backbone1, device, test_loader, 'test', has_labels=False
        )
        
        test_features2, _, _ = extract_features_from_dataloader(
            backbone2, device, test_loader, 'test', has_labels=False
        )
...
def main():
    ...
    # Load checkpoint
    checkpoint1 = load_checkpoint(args.checkpoint1, device=device)
    checkpoint2 = load_checkpoint(args.checkpoint2, device=device)
    ...
    # Build models
    backbone1 = build_backbone(config1, device)
    backbone2 = build_backbone(config2, device)
    
    # Load weights
    load_model_weights(backbone1, checkpoint1)
    load_model_weights(backbone2, checkpoint2)
    
    # Print summary
    print_model_summary(backbone1, backbone2)
    
    crop_size = config1.get('img_size', 96)
    all_results = {}
    ...
    for name, data_dir in datasets_to_eval.items():
        ...
        results = evaluate_dataset(
            backbone1=backbone1, backbone2=backbone2, device=device,
            data_dir=data_dir,
            dataset_name=name,
