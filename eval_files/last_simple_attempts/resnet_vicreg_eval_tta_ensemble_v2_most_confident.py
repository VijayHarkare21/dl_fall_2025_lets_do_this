...
def extract_features_multi_view(images, backbones, device):
    ...
    # Allow a single backbone to be passed for backwards compatibility
    ...
    if images.dim() == 5:
        ...
        for backbone in backbones:
            f = backbone(images)  # (B*V, D_i)
            # Normalize individual views first (same as original)
            f = F.normalize(f, p=2.0, dim=1)
            feats_list.append(f)

        # Concatenate features from all backbones: (B*V, sum_i D_i)
        features = torch.cat(feats_list, dim=1)

        # Reshape back to (B, V, D_total)
        features = features.view(B, V, -1)

        # Average pooling across views
        features = features.mean(dim=1)  # (B, D_total)
        ...
        feats_list = []
        for backbone in backbones:
            f = backbone(images)  # (B, D_i)
            f = F.normalize(f, p=2.0, dim=1)
            feats_list.append(f)

        # Concatenate features from all backbones: (B, D_total)
        features = torch.cat(feats_list, dim=1)
        features = F.normalize(features, p=2.0, dim=1)
...
def adanpc_refinement(
    ...
    If return_confidence=True, also returns per-sample max prob
    (i.e., confidence of the chosen class). This is useful for
    ensembling multiple AdaNPC models by "most confident" selection.
    """
    print(f"\n{'='*40}")
    ...
    conf_list: List[float] = [] if return_confidence else []
    ...
    for idx in range(num_test):
        ...
        exp_scores = np.exp(class_scores - max_score)  # numerical stability
    ...
    preds_arr = np.asarray(final_preds, dtype=np.int64)
    if return_confidence:
        conf_arr = np.asarray(conf_list, dtype=np.float32)
        return preds_arr, conf_arr
    return preds_arr
...
def evaluate_dataset(
    ...
    # TTA increases memory usage, so reduce effective batch size
    ...
    # Normalize backbones input: always treat as list
    if not isinstance(backbones, (list, tuple)):
        backbone_list = [backbones]
    else:
        backbone_list = list(backbones)
    num_backbones = len(backbone_list)

    # ======================================================================
    # CASE 1: SINGLE BACKBONE  -> keep your original concatenation path
    # ======================================================================
    if num_backbones == 1:
        print("Single backbone detected -> using standard concatenation pipeline.")

        print("Extracting features")
        tr_f, tr_l, _ = extract_features_from_dataloader(
            backbone_list, device, train_loader, 'train'
        )
        val_f, val_l, _ = extract_features_from_dataloader(
            backbone_list, device, val_loader, 'val'
        )
        ...
        if has_test and output_csv:
            ...
            test_f, _, test_filenames = extract_features_from_dataloader(
                backbone_list, device, test_loader, 'test', has_labels=False
            )
    ...
    # ======================================================================
    # CASE 2: MULTIPLE BACKBONES  -> confidence-based AdaNPC ensemble
    # ======================================================================
    print(f"\nMultiple backbones detected ({num_backbones}).")
    print("Using per-backbone AdaNPC + confidence-based model selection.")

    # Per-backbone features and k selection
    per_model_train_feats: List[np.ndarray] = []
    per_model_val_feats: List[np.ndarray] = []
    per_model_best_k: List[int] = []
    per_model_best_acc: List[float] = []

    train_labels = None
    val_labels = None

    for idx, backbone in enumerate(backbone_list):
        print(f"\n--- Backbone #{idx}: {type(backbone).__name__} ---")
        # Extract features for this backbone alone (no concatenation across backbones)
        tr_f, tr_l, _ = extract_features_from_dataloader(
            [backbone], device, train_loader, split_name=f'train_b{idx}'
        )
        val_f, val_l, _ = extract_features_from_dataloader(
            [backbone], device, val_loader, split_name=f'val_b{idx}'
        )

        # L2 normalize
        tr_f = tr_f / np.linalg.norm(tr_f, axis=1, keepdims=True)
        val_f = val_f / np.linalg.norm(val_f, axis=1, keepdims=True)

        per_model_train_feats.append(tr_f)
        per_model_val_feats.append(val_f)

        if train_labels is None:
            train_labels = tr_l
            val_labels = val_l

        print(f"Backbone #{idx} -> Train samples: {len(tr_l)}, Val samples: {len(val_l)}, Dim: {tr_f.shape[1]}")
        ...
        for k in k_values:
            ...
            knn.fit(tr_f, train_labels)
            acc = knn.score(val_f, val_labels)
            if acc > best_acc_i:
                best_acc_i = acc
                best_k_i = k
            print(f"  Backbone #{idx} | k={k} | Val Acc: {acc*100:.2f}%")

        print(f"  -> Selected k={best_k_i} for backbone #{idx} (Val Acc={best_acc_i*100:.2f}%)")
        per_model_best_k.append(best_k_i)
        per_model_best_acc.append(best_acc_i)

    # Summarize validation performance across backbones
    print("\nSummary of per-backbone validation performance:")
    for idx in range(num_backbones):
        print(f"  Backbone #{idx}: k={per_model_best_k[idx]}, Val Acc={per_model_best_acc[idx]*100:.2f}%")

    # For overall results, report the best single backbone val accuracy
    best_idx = int(np.argmax(per_model_best_acc))
    results = {
        'best_k': per_model_best_k[best_idx],
        'best_val_accuracy': per_model_best_acc[best_idx],
        'best_k_per_backbone': per_model_best_k,
        'best_val_accuracy_per_backbone': per_model_best_acc,
    }

    print("-" * 35)
    print(f"Best single backbone: #{best_idx} with k={per_model_best_k[best_idx]} "
          f"and val accuracy {per_model_best_acc[best_idx]*100:.2f}%")

    print(f"\n{'='*60}")
    print(f"Generating Submission with confidence ensemble: {dataset_name}")
    print('='*60)

    # --- TEST PREDICTION + per-backbone AdaNPC + confidence ensemble ---
    if has_test and output_csv:
        test_dataset = EvalImageDataset(
            data_dir / 'test', test_df['filename'].tolist(), None, crop_size, use_tta=True
        )
        ...
        per_model_test_feats: List[np.ndarray] = []
        test_filenames = None

        # Extract test features per backbone
        for idx, backbone in enumerate(backbone_list):
            print(f"\nExtracting test features for backbone #{idx}...")
            t_f, _, t_names = extract_features_from_dataloader(
                [backbone], device, test_loader, split_name=f'test_b{idx}', has_labels=False
            )
            t_f = t_f / np.linalg.norm(t_f, axis=1, keepdims=True)
            per_model_test_feats.append(t_f)
            if test_filenames is None:
                test_filenames = t_names

        assert test_filenames is not None

        # Run AdaNPC per backbone and collect preds + confidences
        all_preds = []
        all_confs = []

        for idx in range(num_backbones):
            print(f"\nRunning AdaNPC for backbone #{idx}...")
            full_memory_feats_i = np.concatenate(
                [per_model_train_feats[idx], per_model_val_feats[idx]],
                axis=0
            )
            full_memory_labels_i = np.concatenate([train_labels, val_labels], axis=0)

            preds_i, confs_i = adanpc_refinement(
                train_feats=full_memory_feats_i,
                train_labels=full_memory_labels_i,
                test_feats=per_model_test_feats[idx],
                k=per_model_best_k[idx],
                threshold=adanpc_threshold,
                dataset_name=f"{dataset_name}_b{idx}",
                return_confidence=True,
            )

            all_preds.append(preds_i)
            all_confs.append(confs_i)

        # Stack into shape (N_test, num_backbones)
        all_preds = np.stack(all_preds, axis=1)
        all_confs = np.stack(all_confs, axis=1)

        # For each sample, pick backbone with max confidence
        best_model_idx = np.argmax(all_confs, axis=1)     # (N_test,)
        row_idx = np.arange(all_preds.shape[0])
        final_predictions = all_preds[row_idx, best_model_idx]
