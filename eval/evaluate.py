...
class SevenCrop:
    """
    Generates 7 crops:
    1-5. Standard FiveCrop (TL, TR, BL, BR, Center)
    6. Center Flipped
    7. Global view (resized to crop size)
    """
    def __init__(self, size):
        self.size = size

    def __call__(self, img):
        tl, tr, bl, br, center = TF.five_crop(img, self.size)
        center_flip = TF.hflip(center)
        # Resize original image to crop size for a global view
        global_view = TF.resize(img, [self.size, self.size])
        return (tl, tr, bl, br, center, center_flip, global_view)
...
class EvalImageDataset(Dataset):
    def __init__(
        ...
        # CHANGE 1: Less aggressive zooming.
        # Use 0.95 (instead of 0.875) so the crop preserves more context.
        resize_dim = int(crop_size / 0.95)

        if self.use_tta:
            # --- CHANGE 2: 7-CROP TTA ---
            self.transform = T.Compose([
                T.Resize(resize_dim),
                # Removed CenterCrop so SevenCrop can access corners
                SevenCrop(crop_size),            # Returns tuple of 7 images
                T.Lambda(process_tta_crops)      # Norm & Stack -> (7, 3, H, W)
            ])
        else:
            # --- SINGLE VIEW ---
            self.transform = T.Compose([
...
def collate_fn_labeled(batch):
    images = torch.stack([item[0] for item in batch]) # If TTA: (B, 7, C, H, W)
...
def evaluate_dataset(
    ...
    eff_batch_size = max(1, batch_size // 2)
    ...
    probe, best_val_acc, best_hparams = train_linear_probe_with_search(
        ...
        device=device,
        lrs=[0.5, 1, 1.4, 1.5, 1.6, 1.7, 1.8, 1.9, 5.5, 10.5, 20, 30],
        wds=[0.0],
        epochs=5000,
        batch_size=512,
    )
